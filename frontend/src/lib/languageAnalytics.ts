...
import { trackingLog, devLog } from '@/utils/debug/devLogger';
...
class LanguageAnalytics {
  ...
  trackLanguageUsage(locale: Locale, source: LanguageEvent['source'], metadata?: {
    ...
    if (this.config.debug) {
      trackingLog('LanguageAnalytics: evento registrado', event);
    }
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      if (this.config.debug) {
        trackingLog('LanguageAnalytics: eventos enviados', { count: eventsToSend.length });
      }
    } catch (error) {
      // Erro real: manter em produção, mas com payload enxuto
      devLog.error('[LanguageAnalytics] Erro ao enviar eventos', error);
  ...
  private saveLocally(events: LanguageEvent[]): void {
    try {
      ...
      if (this.config.debug) {
        trackingLog('LanguageAnalytics: eventos salvos localmente', { count: events.length });
      }
    } catch (error) {
      devLog.error('[LanguageAnalytics] Erro ao salvar localmente', error);
    }
  }
  ...
  clear(): void {
    ...
    this.stopFlushTimer();
    trackingLog('LanguageAnalytics: dados limpos');
  }
...
export const languageAnalytics = new LanguageAnalytics({
  enabled: true,
  // Logs só em DEV verbose (evita poluir console em produção)
  debug: import.meta.env.DEV && import.meta.env.VITE_VERBOSE_LOGGING === 'true',
  batchSize: 5,
