...
function buildFunctionsBaseUrl(): string | null {
  ...
  const fallback = 'https://zagkvtxarndluusiluhb.supabase.co';
      ...
      if (supabase?.functions && !signal?.aborted) {
        ...
        try {
          ...
          if (!error && data && (data.language || data.country) && !signal?.aborted) {
            return {
              language: normalizeSimpleLocale(data.language) || 'en',
...
export function normalizeSimpleLocale(lang?: string | null): 'pt' | 'en' | 'es' | null {
  if (!lang) return null;
  const b = lang.slice(0, 2).toLowerCase();
  if (b === 'pt' || b === 'en' || b === 'es') return b;
  return null;
}

function countryToLang(country?: string | null): 'pt' | 'en' | 'es' {
  if (!country) return 'en';
  const c = country.toUpperCase();
  const PT = new Set(['BR','PT','AO','MZ','CV','GW','ST','TL','MO']);
  const ES = new Set(['ES','MX','AR','CO','CL','PE','VE','EC','GT','CU','BO','DO','HN','PY','SV','NI','CR','PA','UY','GQ','PR']);
