...
export default function SongDownload() {
  ...
  const handleAutoDownload = async (songId: string, token: string) => {
    ...
    try {
      ...
      logger.error('Erro no download automático', error);
  ...
  const fetchSong = async () => {
    ...
    try {
      ...
      logger.error('Erro ao buscar música', error);
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        logger.error('Erro ao copiar para área de transferência', error);
  ...
  // ✅ MELHORIA: Mostrar mensagem de sucesso após download automático com instruções claras
  if (autoDownloadComplete) {
    ...
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center gap-6 p-4 bg-gradient-to-br from-primary/5 to-secondary/5" style={{ minHeight: 'var(--dvh)' }}>
            ...
            <div className="flex flex-col items-center gap-6 text-center">
              ...
              <div className="space-y-3">
                <h2 className="text-2xl font-bold text-foreground">
                  {t('songDownload.download.success') || 'Download concluído com sucesso!'}
                </h2>
                <p className="text-base text-foreground leading-relaxed">
                  {t('songDownload.download.instructions') || 'Seu arquivo já foi baixado automaticamente. Procure na pasta de Downloads do seu aparelho (celular, tablet ou computador).'}
                </p>
              </div>
                ...
                >
                  {linkCopied ? (
                    ...
                    <>
                      ...
                      {t('songDownload.copyLink') || 'Copiar Link'}
                    </>
                  )}
                </Button>
              ...
              {/* Dica destacada */}
              <div className="mt-2 p-4 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg w-full">
                <p className="text-sm text-blue-900 dark:text-blue-100 text-left leading-relaxed">
                  <strong className="font-semibold">💡 Dica:</strong> {t('songDownload.download.tip') || 'Se não encontrar o arquivo, verifique a pasta de Downloads do seu dispositivo ou procure pelo nome do arquivo na busca do sistema.'}
                </p>
              </div>
            </div>
          </CardContent>
