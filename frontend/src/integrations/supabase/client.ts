...
// ✅ CORREÇÃO: Builder pattern para queries dummy que suporta chains complexas
function createDummyQueryBuilder() {
  // Criar um objeto que acumula condições e permite múltiplas chamadas encadeadas
  const builder: any = {
    // Métodos de filtro que retornam o próprio builder (permitindo chain)
    ...
    // Métodos de ordenação e limite que também retornam o builder
    order: (column: string, options?: { ascending?: boolean }) => builder,
    limit: (count: number) => builder,
    range: (from: number, to: number) => builder,
    abortSignal: (signal: AbortSignal) => builder,
    
    // Métodos finais que executam a query (retornam Promise)
...
function createDummyClient(): any {
  ...
  return {
    ...
    from: (table: string) => ({
      // ✅ CORREÇÃO: select() retorna o builder que suporta múltiplas chamadas encadeadas
      select: (columns?: string) => createDummyQueryBuilder(),
      insert: async () => ({ data: null, error: null }),
      update: async () => ({ data: null, error: null }),
      upsert: async () => ({ data: null, error: null }),
      delete: async () => ({ data: null, error: null }),
