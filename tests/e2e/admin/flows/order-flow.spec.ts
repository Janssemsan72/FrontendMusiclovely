...
test.describe('Order Complete Flow', () => {
  ...
  test('deve manter contexto entre navegações', async ({ page }) => {
    ...
    const statusFilterTrigger = page.locator('[data-testid="filter-status"]');
    await expect(statusFilterTrigger).toBeVisible();
    await expect(statusFilterTrigger).toHaveText(/Todos|Pago|Status/i);
  });
  ...
  test('deve exibir informações consistentes entre páginas', async ({ page }) => {
    ...
    if ((await ordersStats.count()) > 0) {
      await expect(ordersStats).toBeVisible();
    }
    if ((await revenueStats.count()) > 0) {
