...
test.describe('Admin Responsive Design', () => {
  ...
  test.describe('Mobile Viewport (375x667)', () => {
    ...
    test('deve adaptar página de pedidos para mobile', async ({ page }) => {
      await page.goto('/admin/orders', { waitUntil: 'commit', timeout: 60000 });
      await page.waitForSelector('main', { timeout: 30000 });
      ...
      if (await orderRows.count() === 0) {
        await expect(page.getByText('Nenhum pedido encontrado')).toBeVisible({ timeout: 15000 });
        return;
      }
    ...
    test('deve adaptar página de músicas para mobile', async ({ page }) => {
      await page.goto('/admin/songs', { waitUntil: 'commit', timeout: 60000 });
      ...
      const emptyState = page.getByText('Nenhuma música encontrada');
      if (await songGroups.count() === 0) {
        await expect(emptyState).toBeVisible({ timeout: 15000 });
        return;
      }
      await expect(songGroups.first()).toBeVisible({ timeout: 15000 });
    ...
    test('deve adaptar wizard de geração para mobile', async ({ page }) => {
      await page.goto('/admin/generate', { waitUntil: 'commit', timeout: 60000 });
      await page.waitForSelector('main', { timeout: 30000 });
  ...
  test.describe('Tablet Viewport (768x1024)', () => {
    ...
    test('deve adaptar sidebar para tablet', async ({ page }) => {
      ...
      const isCollapsed = await sidebar.evaluate(el => el instanceof HTMLElement ? el.offsetWidth < 200 : false);
      expect(isCollapsed).toBe(true);
    });

    test('deve adaptar dashboard para tablet', async ({ page }) => {
      // Verificar se cards se organizam em grid
      const statsCards = page.locator('main [role="region"][aria-label]');
  ...
  test.describe('Desktop Viewport (1920x1080)', () => {
    ...
    test('deve exibir layout completo em desktop', async ({ page }) => {
      ...
      const isExpanded = await sidebar.evaluate(el => el instanceof HTMLElement ? el.offsetWidth > 200 : false);
