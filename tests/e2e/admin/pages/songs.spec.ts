...
test.describe('Admin Songs', () => {
  ...
  test('deve filtrar músicas por período', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(count, 5); i++) {
      const group = songGroups.nth(i);
      const dateElement = group.locator('[data-testid="group-date"]');
  ...
  test('deve exibir loading states', async ({ page }) => {
    ...
    if ((await pageLoading.count()) > 0) {
      await expect(pageLoading).not.toBeVisible();
    }
  });
  ...
  test('deve permitir ordenação por data de release', async ({ page }) => {
    ...
    if (count > 1) {
      ...
      const firstDateObj = firstDate ? parsePtBrDate(firstDate) : null;
      const secondDateObj = secondDate ? parsePtBrDate(secondDate) : null;
      if (!firstDateObj || !secondDateObj) return;
